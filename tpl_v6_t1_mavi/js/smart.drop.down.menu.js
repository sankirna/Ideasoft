	...
	*/
	...
	jQuery.fn.SmartyDropMenu = function(opt) {
		var opt = jQuery.extend({
			dropMenuSubCategoryWidth: 320, /*  320[2], 480[3], 640[4], 800[5] px vs vs.. */
			numberOfColumns: 2,
			extraLeftSpace: 1, /* Ana Link 'de border varsa sub category 'de bulunan boslugu duzenltebiliriz. */
			dropMenuEffect: 'fade',
			InEffect : 'easeInBounce',
			OutEffect : 'easeOutBounce',
			speedIn: 500,
			speedOut: 200,
			/* Html Selectors */
			...
			/* Category Settings */
			showOrHideThirdLevelCategory: true,
			countThirdLevelCategories: 2,
		...
		CategoriesTripping = function(event) {
			switch(event){
				case "fade" : jQuery(el).find(' > ul > li').hover(function(){
					...
					if(jQuery(this).find(opt.dropMenuSubCategorySelector + ' > *').length > 0){
				...
				case "show" : jQuery(el).find(' > ul > li').hover(function(){
					dropMenuSubCategoryPosition(this);
					if(jQuery(this).find(opt.dropMenuSubCategorySelector + ' > *').length > 0){
						jQuery(this).find(opt.dropMenuSubCategorySelector).stop().animate({
							height:"toggle"       
